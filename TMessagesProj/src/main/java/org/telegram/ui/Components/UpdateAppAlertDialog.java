...
public class UpdateAppAlertDialog extends BottomSheet {
    ...
    public UpdateAppAlertDialog(Context context, TLRPC.TL_help_appUpdate update, int account) {
        ...
        if (appUpdate.sticker != null) {
            ...
            if (svgThumb != null) {
                imageView.setImage(ImageLocation.getForDocument(appUpdate.sticker), "tgs", svgThumb, 0, "update");
            } else {
                imageView.setImage(ImageLocation.getForDocument(appUpdate.sticker), "tgs", imageLocation, null, 0, "update");
            }
